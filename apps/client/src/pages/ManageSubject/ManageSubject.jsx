...
const ManageSubject = () => {
    ...
    const { accessibleColleges: colleges, classes, departments, loaded: lookupsLoaded } = useSelector(state => state.lookup);
    ...
    // Form state
    const [formSubject, setFormSubject] = useState({ name: '', code: '', classId: '', departmentId: '' });
    ...
    // Clear dependencies if college changes
    useEffect(() => {
        if (!editingItem) {
            setFormSubject(prev => ({ ...prev, classId: '', departmentId: '' }));
        }
    }, [selectedCollegeId]);

    const fetchSubjects = async (collegeId) => {
        if (!collegeId && isMultiCollegeRole) return;
        setIsFetchingData(true);
        try {
            const res = await academicService.getEntities('subjects', collegeId);
    ...
    useEffect(() => {
        if (selectedCollegeId) {
            fetchSubjects(selectedCollegeId);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await academicService.deleteEntity('subjects', id);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const payload = { ...formSubject, collegeId: selectedCollegeId };
            if (editingItem) {
                await academicService.updateEntity('subjects', editingItem._id, payload);
                toast.success("Updated successfully");
            } else {
                await academicService.createEntity('subjects', payload);
                toast.success("Created successfully");
            }
            fetchSubjects(selectedCollegeId);
            dispatch(fetchLookups(selectedCollegeId));
            setFormSubject({ name: '', code: '', classId: '', departmentId: '' });
            setEditingItem(null);
            if (!editingItem) {
                setIsFormOpen(false);
            }
    ...
    const displayData = useMemo(() => {
        ...
        return subjectsData.filter(item => String(item.collegeId?._id || item.collegeId) === String(selectedCollegeId));
    }, [subjectsData, selectedCollegeId, isMultiCollegeRole]);
    ...
    return (
        <PageLayout title="Manage Subjects" description="Create and view academic subjects.">
            <div className="flex flex-col gap-6 animate-in slide-in-from-bottom-4">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-xl font-bold text-gray-800">
                                {editingItem ? 'Edit Subject' : 'Create New Subject'}
                            </h2>
                        </div>
                            ...
                            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-5 items-end">
                                ...
                                <div className="col-span-1 md:col-span-1">
                                    <InputField 
                                        required 
                                        label="Subject Code" 
                                        value={formSubject.code} 
                                        onChange={e => setFormSubject({ ...formSubject, code: e.target.value })} 
                                ...
                                <div className="col-span-1 md:col-span-2">
                                    <SelectField 
                                        ...
                                        value={formSubject.classId} 
                                        onChange={e => setFormSubject({ ...formSubject, classId: e.target.value })}
                                    >
                                ...
                                <div className="col-span-1 md:col-span-1">
                                    <SelectField 
                                        ...
                                        value={formSubject.departmentId} 
                                        onChange={e => setFormSubject({ ...formSubject, departmentId: e.target.value })}
                                    >
                                        <option value="">None</option>
                                ...
                                <div className="col-span-1 md:col-span-1 flex gap-3 pb-1">
                ...
                <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                        <h2 className="text-xl font-bold text-gray-800">Subjects List</h2>
                        {isMultiCollegeRole && (
                    ...
                    <DataTable
                        ...
                        columns={[
                            ...
                            { header: 'Subject Code', accessor: row => row.code },
