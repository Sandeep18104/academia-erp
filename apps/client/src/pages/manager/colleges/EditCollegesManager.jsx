...
const EditCollegesManager = ({ refreshList, editingCollege, setEditingCollege, onClose }) => {
    ...
    const handleSaveFeatures = async (features) => {
        ...
        try {
            ...
            refreshList();
            setEditingCollege({ ...editingCollege, features });
        } catch (error) {
    ...
    return (
        <div className="bg-white p-8 rounded-3xl border border-gray-100">
            <h2 className="text-2xl font-bold mb-6 text-gray-800">{editingCollege ? 'Edit Institute' : 'Onboard Direct Institute'}</h2>
            <form onSubmit={handleOnboard} className="space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                    ...
                    <PrimaryButton type="submit" loading={loading} className={editingCollege ? 'flex-1' : 'w-full'}>
                        {editingCollege ? 'Update Institute' : 'Onboard & Generate Credentials'}
                    </PrimaryButton>
                </div>
            </form>
